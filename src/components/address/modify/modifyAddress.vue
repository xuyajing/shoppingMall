<template>
  <transition name="move">
    <div class="modifyAddressWrap">
      <top-header :title="title"></top-header>
      <form class="modifyAddressForm">
        <ul>
          <li class="item">
            <label>收件人</label>
          ...
          <li class="item">
            ...
            <div class="areaWrap rightWrap border-1px">
              <span type="text" name="area" @click.stop.prevent="showSelectArea" class="areaBox">{{provinceName}}{{cityName}}{{districtName}}</span>
        ...
        <a class="btnConfirm" @click.stop.prevent="modifyAddress">确定</a>
      </form>
...
<script type="text/ecmascript-6">
  ...
  import {updateAddress} from '../../../service/getData';
...
//              addressInfo: {},
      ...
      computed: {
        ...
        ...mapState({
          addressInfo(state) {
            return state.addressList.filter((item) => item.id === this.$route.query.id)[0];
          },
          provinceList: state => {
      ...
      activated() {
          this.addressTableId = this.$route.query.id;
          this.token = getStore('token');
          this.name = this.addressInfo.consigneeName;
          this.phone = this.addressInfo.consigneeTel;
          ...
          this.isDef = this.addressInfo.isDef;
      },
      methods: {
          ...mapMutations([
              'UPDATE_ADDRESS'
          ...
          async modifyAddress() {
              let updateAddressResult = await updateAddress(this.addressTableId, this.token, this.name, this.phone, this.provinceId, this.cityId, this.districtId, this.address, this.isDef);
              if (updateAddressResult.msg === '成功') {
                  this.UPDATE_ADDRESS({
                      id: this.addressTableId,
